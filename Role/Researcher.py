...
from Action.SearchInWeb import CollectLinks, ConductResearch, WebBrowseAndSummarize, get_research_system_text
...
class Researcher(Role):
    ...
    async def _act(self) -> Message:
        ...
        research_system_text = self.research_system_text(self.status, topic, todo)
        if isinstance(todo, CollectLinks):
            links = await todo.run(topic, 4, 4, research_system_text)
            ret = Message(
    ...
    def research_system_text(self, status, topic, current_task: Action) -> str:
        """BACKWARD compatible
        ...
        Returns: str
        """
        return get_research_system_text(status, topic, self.language)
